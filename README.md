...
Jika nama_baru == "", program menampilkan pesan bahwa nama tugas tidak boleh kosong dan meminta pengguna memasukkan nama kembali. Setelahnya, Program meminta pengguna memasukkan jumlah hari yang tersisa sebelum deadline yang baru. Nilai tersebut disimpan dalam variabel sisa_hari_baru. Dan akan melakukan sistem validasi apakah kode yang digunakan merupakan huruf atau angka.( Untuk sistem ini input harus angka). Setelah input dipastikan berupa angka, nilai tersebut diubah dari string menjadi integer menggunakan int().Setelah itu, bagian ini menampilkan pilihan tingkat kesulitan tugas dari skala 1 sampai 5. Pengguna dapat menentukan tingkat kesulitan tugas yang baru berdasarkan pilihan tersebut.<br>.
